...
uint16_t CRC16_Modbus(uint8_t* data, uint8_t length){
    ...
    {
        index = CRCHi ^ *data++;  /* calculate the CRC */
...
/**
 * @func   CRC16_Kermit
 * @brief  calculate crc16 kermit
 * @param  data, length
 * @retval crc16 value
 */

uint16_t CRC16_Kermit(uint8_t *data, uint8_t length){
	uint16_t i, j, crc = 0x3791;      // According manufacturer
	crc = 0;                          // Normaly
	...
	for( i = 0; i < length; i++){
		...
		for( j = 0; j < 8; j++){
			if( crc & 0x0001){
				crc = (crc >> 1) ^ 0x8408; // if rightmost (most significant) bit is set
			}				
			else{
				crc = crc >> 1;
			}
